        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="col-span-1 md:col-span-2">
            ...
            <p className="text-gray-400 mb-4">
              La plataforma de retos personales con validación comunitaria. Crece, comparte y
              alcanza tus metas con el apoyo de una comunidad comprometida.
            </p>
        ...
        <div className="border-t border-gray-800 pt-8">
          ...
          <div className="text-center mb-6">
            <p className="text-gray-400 mb-2">
              <strong className="text-white">{LEGAL_COPY.company.name}</strong> – nombre comercial
              de {LEGAL_COPY.company.owner}
            </p>
            <p className="text-gray-400 mb-4">{LEGAL_COPY.company.address}</p>
          ...
          {process.env.NODE_ENV === "development" && (
            <div className="bg-gray-800 rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                <div
                  className={`px-2 py-1 rounded ${config.isBillingEnabled ? "bg-green-600" : "bg-red-600"}`}
                >
                  BILLING: {config.isBillingEnabled ? "ON" : "OFF"}
                </div>
                <div
                  className={`px-2 py-1 rounded ${config.isCoachMarketEnabled ? "bg-green-600" : "bg-red-600"}`}
                >
                  COACH: {config.isCoachMarketEnabled ? "ON" : "OFF"}
                </div>
                <div
                  className={`px-2 py-1 rounded ${config.areAdsEnabled ? "bg-green-600" : "bg-red-600"}`}
                >
                  ADS: {config.areAdsEnabled ? "ON" : "OFF"}
                </div>
                <div
                  className={`px-2 py-1 rounded ${config.isCMPEnabled ? "bg-green-600" : "bg-red-600"}`}
                >
          ...
          <div className="text-center text-sm text-gray-500">
            <p className="mb-2">
              © {new Date().getFullYear()} {LEGAL_COPY.company.name}. Todos los derechos
              reservados.
            </p>
            <p>IMPULSE LEAN v1.0.0 - Sistema de Retos y Validación Comunitaria</p>
