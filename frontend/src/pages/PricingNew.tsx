...
import { PRICING_PLANS, PRICING_COPY, formatPrice, getAllPlans, isPlanAvailable } from '../config/pricing';
...
const Pricing: React.FC = () => {
  ...
  const getIcon = (planId: string) => {
    switch (planId) {
      ...
      case 'teams': return <Users className="h-6 w-6" />;
      case 'coach': return <Crown className="h-6 w-6" />;
      ...
      <div className="max-w-7xl mx-auto">
              ...
              <CardHeader className="text-center pb-2">
                ...
                {plan.teamConfig && (
                  <p className="text-sm text-gray-600 mt-1">
                    ...
                    <br />
                    +{formatPrice(plan.teamConfig.extraMemberPrice)}/miembro extra
                  </p>
                )}
              </CardHeader>
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                <tr>
                  ...
                  {availablePlans.map(plan => (
                    <td key={plan.id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                      {plan.limits.activeChallenges === 'unlimited' ? '∞' : plan.limits.activeChallenges}
                    </td>
                  ))}
                </tr>
                ...
                <tr className="bg-gray-50">
                  ...
                  {availablePlans.map(plan => (
                    <td key={plan.id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                      {plan.limits.validatorsPerChallenge === 'unlimited' ? '∞' : plan.limits.validatorsPerChallenge}
                    </td>
                  ))}
                </tr>
        ...
        <div className="text-center text-sm text-gray-600 bg-gray-50 rounded-lg p-6">
          ...
          <div className="space-y-2">
            ...
            <p>{config.address}</p>
            <p>
              Soporte: <a href={`mailto:${config.supportEmail}`} className="text-blue-600 hover:underline">
                {config.supportEmail}
              </a> · 
              RGPD: <a href={`mailto:${config.legalEmail}`} className="text-blue-600 hover:underline">
                {config.legalEmail}
              </a> · 
              Abusos: <a href={`mailto:${config.abuseEmail}`} className="text-blue-600 hover:underline">
                {config.abuseEmail}
              </a>
            </p>
          </div>
